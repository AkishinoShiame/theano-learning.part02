...
for i in range(200):
    print "reading", i, "..."
    pil_im = Image.open( "Training_data 2.0/" + thelist[i] + ".jpg" ).convert('L')
    #imshow(np.asarray(pil_im)) # before resize
    pil_im = pil_im.resize((28, 28), Image.BILINEAR )
    pil_im = np.array(pil_im)
    fig = plt.figure()
    ...
    for j in range(28):
        for k in range(28):
            final_output[i+200][note]= ((255 - pil_im[j][k])/225.)
            note += 1
            print " in ", note, "...",
...
f = gzip.open("training_data_400v3.pkl.gz", 'wb')
